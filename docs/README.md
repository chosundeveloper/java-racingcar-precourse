...
#### 경주 자동차
#### 속성
- `경주 자동차(racing cars)`는 `자동차(car)`들과 `이동 횟수(moving number)`를 갖는다.
#### 기능
- `경주 자동차(racing cars)`는 `차수 진행(roundPlay)`을 한다.
  - `경주 자동차(racing cars)`는 `이동 횟수(moving number)`만큼 `차수 진행(roundPlay)`을 한다.
#### 우승 자동차
#### 속성
